...
char *fsm_dump_string(char *s) {
  ...
  for (i=0; i<n; i++) buf[i] = fsm_dump_char(s[i]);
  buf[i] = 0;
  return buf;
}
 ...
 *------------------------------------------------------------------*/
extern
void fsm_print_error(char *errorstring, ...) {
 ...
 *------------------------------------------------------------------*/
extern
int fsm_get_next_char(char *b, int UNUSED(max_buffer)) {
  int frc;
  ...
  if (debug)
    printf("get_next_char() => '%c'0x%02x at %d\n",
                        fsm_dump_char(b[0]), b[0], flex_n_buffer);
  return b[0]==0?0:1;
}
...
void fsm_begin_token(char *t) {
  ...
  /*================================================================*/
  /* location for bison --------------------------------------------*/
  fsm_lloc.first_line = flex_n_row;
  fsm_lloc.first_column = flex_n_token_start;
  fsm_lloc.last_line = flex_n_row;
  fsm_lloc.last_column = flex_n_token_start + flex_n_token_length - 1;

  if (debug) {
    printf("Token '%s' at %d.%d next at %d\n", fsm_dump_string(t),
                        fsm_lloc.first_column,
                        fsm_lloc.last_column, flex_n_token_next_start);
  }
}
 ...
 *----------------------------------------------------------------------------*/
extern
void fsm_error(grammar_t * UNUSED(fsm), char *s) {
  fsm_print_error(s);
}
...
int grammar_load_fsm(grammar_t *fsm, FILE *file) {
  ...
  /*================================================================*/
  /* parse it ------------------------------------------------------*/
  if (fsm_get_next_line() == 0) fsm_parse(fsm);
